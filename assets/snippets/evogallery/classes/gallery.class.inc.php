...
class Gallery
{
        ...
        */
        function renderGalleries()
        {
                ...
                if ($this->config['tpl'] == '')
                {$tpl = file_get_contents($this->config['snippetPath'] . $this->config['type'] . '/tpl.default.txt');}
                elseif (substr($this->config['tpl'], 0, 6) == "@CODE:")
                {$tpl = substr($this->config['tpl'], 6);}
                else {$tpl = $modx->getChunk($this->config['tpl']);}
                
                if ($this->config['itemTpl'] == '')
                {$item_tpl = file_get_contents($this->config['snippetPath'] . $this->config['type'] . '/tpl.item.default.txt');}
                elseif (substr($this->config['itemTpl'], 0, 6) == "@CODE:")
                {$item_tpl = substr($this->config['itemTpl'], 6);}
                else {$item_tpl = $modx->getChunk($this->config['itemTpl']);}
